...
#ifndef _GAUSSIANFILTER_COSIM_INCLUDED_
#define _GAUSSIANFILTER_COSIM_INCLUDED_
...
#ifdef NC_SYSTEMC
struct GaussianFilter_cosim : public ncsc_foreign_module
#else
SC_MODULE(GaussianFilter_cosim)
#endif
{
	struct StringPair {
		...
		if ( nin == 0 )
		{
			if ( numLinked > 0 ) 
			{
				esc_report_error( esc_fatal, 
					"More than one instance of module GaussianFilter is being simulated in Verilog,\n"
					...
					"where <inst1> and <inst2> are the leaf names of instances of GaussianFilter in SystemC\n", 
					scName );
				return;
			} else {
				s << "GaussianFilter0" << std::ends;
			}
		}
		else
		{
			...
			{
				esc_report_error( esc_fatal, 
								  "More than %d instance(s) of module GaussianFilter are being simulated in Verilog,\n"
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		if ( i_clk_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "GaussianFilter", "i_clk", scName ) ) {
			esc_report_error( esc_error, "The port 'i_clk' on module 'GaussianFilter'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( i_rst_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "GaussianFilter", "i_rst", scName ) ) {
			esc_report_error( esc_error, "The port 'i_rst' on module 'GaussianFilter'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( i_rgb_vld_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "GaussianFilter", "vld", scName ) ) {
			esc_report_error( esc_error, "The port 'vld' on module 'GaussianFilter'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( o_result_busy_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "GaussianFilter", "busy", scName ) ) {
			esc_report_error( esc_error, "The port 'busy' on module 'GaussianFilter'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		...
		{
			esc_report_error( esc_fatal, 
				"The clock port in module GaussianFilter must have\n"
...
#ifdef NC_SYSTEMC
    const char* hdl_name() const { return "GaussianFilter_nc_cosim"; } 
    GaussianFilter_cosim( sc_module_name name )
        : ncsc_foreign_module(name)
#else
	GaussianFilter_cosim( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" GaussianFilter") ) )
		: sc_module(in_name)
#endif
		  ,i_clk("i_clk")
		  	,i_rst("i_rst")
...
int GaussianFilter_cosim::numLinked = 0;
GaussianFilter_cosim::StringPair GaussianFilter_cosim::instanceNames[] = {
	{ NULL, NULL, false } };
